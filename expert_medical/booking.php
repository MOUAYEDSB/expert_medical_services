...
if (isset($_POST['submit'])) {
  ...
  // Assuming 'number' is a VARCHAR field; adjust if it's a different type
  $stmt = $conn->prepare("INSERT INTO book (fullname, email, number, birth, gender, country, city, postalcode, services, serviceDetails, accommodation, status)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");

  // Check if the prepare statement is successful
  if ($stmt) {
    $stmt->bind_param("ssssssssssss", $fullname, $email, $number, $birth, $gender, $country, $city, $postalcode, $services, $serviceDetails, $accommodation, $status);

    if ($stmt->execute()) {
      $message = "We received your booking for a service!";
    } else {
...
<body>
  ...
  <div class="page_content">
    <section class="first-container">
      <h3>Health Voyage : Booking and details</h3>
      <p>Please take a moment to outline your requirements below. Our team of esteemed professionals
        is ready to ensure that your healthcare journey with Health Voyage is as precise, personalized,
    ...
    <section class="container-two">
      ...
      <span>Please verify your details before submitting;
      ...
      <form class="form" method="POST" action="">
        ...
        <br />
        <a href="payment.php">
          <p>Pay now 50% of your service tarification to accelerate the registration process!</p>
        </a>
        <div class="column">
